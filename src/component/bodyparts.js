...
const Bodyparts = () => {
    const { data: { bodyPartList } } = useContext(MyContext)
            ...
            <div className="d-flex w-100  justify-content-center flex-wrap p-2" >
                {bodyPartList.slice(pre1, next1).map((item, index) => {
                    return <div key={index} className="w-25 my-4 card border-2 p-2" >
                        ...
                        <button onClick={() => { fetch(item) }} className="text-center text-responsive p-1 border-1 border-danger border text-capitalize">{item}</button>
                    </div>
