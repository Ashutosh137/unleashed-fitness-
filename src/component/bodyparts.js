...
const Bodyparts = () => {
  ...
  const slider =
    bodyPartList.length % 4 === 0
      ? bodyPartList.length / 4
      : bodyPartList.length / 4 + 1;
    ...
    <div className="m-auto bg-light rounded ">
      <div className="d-flex w-100  justify-content-center flex-wrap p-3">
        {bodyPartList.slice(pre1, next1).map((item, index) => {
          return (
            <div key={index} className="w-25  my-4 card border-2 py-4">
              <img
                className="img-fluid mb-3"
                src="https://t4.ftcdn.net/jpg/02/19/15/99/240_F_219159923_iCk4Qp5HIlSU7eflsShBFMYwkutGMm9v.jpg"
                alt=""
              />
              <button
                onClick={() => {
                ...
                className="text-center text-responsive  p-1 w-75 m-auto border-1 border-danger border text-capitalize"
              >
                {item || <Skeleton />}
              </button>
            </div>
