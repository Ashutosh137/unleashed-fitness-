...
const Home = () => {
  ...
  const handelsubmit = (e) => {
    ...
    const searchdata = allexercise.filter((item) => {
      return (
        item.name?.toLowerCase() === searchLowerCase ||
  ...
  return (
    <div className="pb-5 text-responsive rounded text-capitalize">
      <div className="d-flex align-middle home">
        <div className="m-auto main d-flex w-50 h-50 rounded p-5 text-center">
          <div className="m-auto">
            <h1>sweet ,smile and repeat </h1>
            <hr />
            <p>Train with Passion, Achieve Greatness</p>
            <button className="btn btn-danger m-2 text-capitalize">
              <a href="#main" className="text-decoration-none text-light">
                get started
              </a>
            </button>
          </div>
      ...
      <div className="my-4 py-2">
        <h1 className="text-center h2 my-4">
        ...
        <form onSubmit={(e) => handelsubmit(e)} className="d-flex ">
          <input
            type="search"
            name="search"
            value={searchLowerCase}
            onChange={(e) => {
              setsearch(e.target.value.toLowerCase());
            }}
            className="text-dark p-3 m-2 rounded w-100"
            placeholder="Search your favirate exercise by name, bodyparts, equipment or target muscles"
          />
          <button type="submit" className="btn btn-danger m-2 fw-semibold">
            search
          </button>
        </form>
      ...
      <div id="main" className="mx-2">
        <div className="my-4">
          <h3 className=" h2 m-auto my-4 border-2 border-bottom border-dark p-2">
            search by bodyparts
          </h3>
          <Bodyparts />
        </div>
