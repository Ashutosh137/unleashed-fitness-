...
const Muscles = () => {
  ...
  const slider =
    targetList.length % 4 === 0
      ? targetList.length / 4
      : targetList.length / 4 + 1;
          ...
          return (
            <div key={index} className="w-25 my-4 card border-2 p-4">
              <img
                ...
                src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQyMyQqOKCPUxwvCBKi_WXiiNSnPhJmgoqRDQ&usqp=CAU"
                alt=""
              />
              <button
                onClick={() => {
                ...
                className="text-responsive w-100  m-auto text-center text-capitalize p-1 border-1 border-danger border"
              >
                {item || <Skeleton />}
              </button>
            </div>
