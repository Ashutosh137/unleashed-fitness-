...
const Allexercise = () => {
  ...
  const filter = async (bodyPart, equipment, target) => {
    ...
    const searchdataall = await allexercise.filter((item) => {
      return (
        equipment.includes(item.equipment?.toLowerCase()) &&
        target.includes(item.target?.toLowerCase()) &&
        bodyPart.includes(item.bodyPart?.toLowerCase())
      );
    });
    toggleload();
  ...
  return (
    <div className="w-100 rounded text-responsive d-flex flex-column bg-light">
      <h4 className="text-center text-capitalize mx-5 bg-light border-bottom border-1 border-dark p-4 m-5 h1 text-suceess">
        all exercise
      </h4>
      <button
        className="px-3 btn-primary text-dark w-25 m-auto text-capitalize"
        type="button"
      ...
      <div className="w-100">
        <div className=" p-5">
          {data[0] ? (
            <Search exercise={data} n={6} />
          ) : (
            <h1 className="text-center text-capitalize bg-light p-5 text-suceess">
              no result found
            </h1>
          )}
        </div>
      ...
      <div className="offcanvas offcanvas-end" id="filter">
        ...
        <div className="w-100 overflowx-hidden">
          <form
            className="w-100 text-capitalize absolute-filter rounded "
            onSubmit={(e) => {
              e.preventDefault();
              ...
              const bodyPartlabels = Array.from(bodyParts).map((checkbox) =>
                checkbox.nextElementSibling.innerText.toLowerCase()
              );
              const equipmentlabels = Array.from(equipment).map((checkbox) =>
                checkbox.nextElementSibling.innerText.toLowerCase()
              );
              const targetlabels = Array.from(target).map((checkbox) =>
                checkbox.nextElementSibling.innerText.toLowerCase()
              );
          ...
          >
            ...
            <details className="list-none border border-2 border-danger m-2  rounded p-2 bg-light">
              <summary className="border-bottom border-2 border-danger mb-2">
                <h5>according to bodyparts</h5>
              </summary>
              <div className="d-flex flex-column">
                {bodyPartList.map((item, index) => {
                  return (
                    <div key={index}>
                      <input
                        className="btn curser-pointer mx-2 h5"
                        type="checkbox"
                        id="bodyparts"
                      />
            ...
            <details className="list-none border border-2 m-2 border-danger  rounded p-2 bg-light">
              <summary className="border-bottom border-2 border-danger mb-2">
                <h5>according to target muscles</h5>
              </summary>
            ...
            <div className="d-flex justify-content-center my-3 curser-pointer">
              <button
                type="reset"
                className="btn mx-1 text-capitalize fw-semibold  btn-danger"
                data-bs-dismiss="offcanvas"
                onClick={() => {
                  setdata(allexercise);
                }}
